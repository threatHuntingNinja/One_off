...
do_arp = "no"; do_tcp = "yes"; do_icmp = "yes"; do_udp = "no"; # TEST
display("let's get this party started right\n");
...
if ( can_use_new_engine() )
{
 ...
 p = make_list(80,139,445,443);			
 display("setting ports to 80,139,443,445\n");
 res = ping_host4(tcp_ports:p);
 display(string("res is '",res,"'\n"));
 if ( !isnull(res) )
...
if("yes" >< do_tcp)
{
 ...
 foreach dport (split(p, sep: ';', keep: 0))
 {
   ...
   if (dport == "built-in")
   {
     ...
     {
       t2 = gettimeofday();
       display('Host answered to TCP SYN (built-in port list)\n');
   ...
   else
   {
     ...
     {
       t2 = gettimeofday();
       display('Host answered to TCP SYN on port ', dport, '\n');
...
if(retry <= 0) retry = 2;	# default
  ...
  display("ICMP retry count=", retry, "\n");
  j = 0;
  test = test + 1;
  icmpid = rand() % 0xFFFF;
  ...
  while(j < retry)
  {
   ...
   if(rep){
	...
	display(get_host_ip(), ' answered to ICMP ping\n');
...
#
 ...
 display('sending ', n, ' UDP requests\n');
 ...
 for (j = 0; j < n; j ++)
 {
  ...
  for (i = 0; i < 3; i ++)	# Try 3 times
  {
   ...
   if (r)
   {
    ...
    display('Host answered to UDP request on port ', dstports[j], ' (protocol=', ipp, ')\n');
    if (ipp == 17)
    {
